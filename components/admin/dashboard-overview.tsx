...
export function DashboardOverview() {
  ...
  const metrics = [
    { label: "Active Riders", value: String(data.metrics.active_riders), icon: Bike },
    { label: "Live Orders", value: String(data.metrics.live_orders), icon: PackageOpen },
    { label: "COD Pending Cash", value: `₨ ${Number(data.metrics.cod_pending).toLocaleString("en-PK")}`, icon: Banknote },
    { label: "Bank Approvals Pending", value: String(data.metrics.bank_approvals), icon: ShieldCheck },
  ]
    ...
    <div className="space-y-6">
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
        {metrics.map((m) => {
          const Icon = m.icon
          return (
            <div key={m.label} className="rounded-2xl border border-border bg-gradient-to-br from-accent/60 to-card p-5">
